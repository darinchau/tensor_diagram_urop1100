...
Original file is located at
    https://colab.research.google.com/drive/1IWYkjczP-WcawFXd7IFNgca9xBTkbZrg

Note to whoever decided to pull this: This code is seriously not optimized. The UROP project has a rather tight deadline and I have to manage this on top of all the course work, and I just didn't have time to write optimal algorithms for everything. Since we are only dealing with small cases, I think some of the multi-layered for loops does not affect the process that much.
"""
  ...
  def __generateStrContents__(self):
    ...
    for n in self.nodes:
      for id in n.connected:
        ...
        # Check and see if node is already drawn
        lineID = set([n.id, id])
        filter = [d for d in drawnLines if d == lineID]
        # This means the line is already drawn
        if len(filter) > 0: continue

        st += f"\n\t\\draw[black, thick] {n.coordinate()} -- {toNode.coordinate()};"
        drawnLines.append(lineID)
    
    for n in self.nodes:
      if n.color == w:
        st += f"\n\t\\filldraw[color=black, fill=white] {n.coordinate()} circle (0.8pt);"
      elif n.color == b:
        st += f"\n\t\\filldraw[color=black] {n.coordinate()} circle (0.8pt);"
...
\t\\filldraw[color=black, fill=white!0, thin](0,0) circle (1);"""
  ...
  def __fakeStr__(self):
    # We use a more lenient approach to prevent slight errors
    self.OptimizeDiagram(divs = 100)
    st = set()
    #Now draw the lines
    drawnLines = []
    for n in self.nodes:
      for id in n.connected:
        ...
        # Check and see if node is already drawn
        lineID = set([n.id, id])
        if Contains(drawnLines, lineID): continue

        st.add(f"line {n.coordinate( precision = 3 )} -- {toNode.coordinate( precision = 3 )}")
        st.add(f"line {toNode.coordinate( precision = 3 )} -- {n.coordinate( precision = 3 )}")

        drawnLines.append(lineID)
    
    # Only do internal nodes since we already established boundary nodes are the same via signature verification
    for n in self.nodes:
      if n.color == w:
        st.add(f"white {n.coordinate( precision = 3 )}")
      elif n.color == b:
        st.add(f"black {n.coordinate( precision = 3 )}")
    return st


    # We try to come up with an algorithm that determines two diagrams are equivalent (i.e. they represent the same element in R_sigma(V))
    # They are the same (up to sign) if they can be transformed into one another by a relabelling of internal vertices.
    # To simplify the process, we assume they have the same signature for now
  def __eq__(self, other):
    # Their basic properties must be the same
    if type(self) != type(other) or self.signature != other.signature:
      return False
    # I am told the graph isomorphism problem is np hard in general so we use a slightly shady approach: if both graphs, after the optimization algorithm, turns out to be the same thing up to relabelling, then we claim they are the same
    # We assume there are no double lines since graphs with double edges are non-elliptic and we yet to have an algorithm to handle those
    # Sure there will be false positives as the graph gets very large but it is probably sufficient for the intents of this project
    # And we will manually verify each of the outputs
    set1 = self.__fakeStr__()
    set2 = other.__fakeStr__()
    return set1 == set2
...
def GenerateQMUfile(B_matrix : np.ndarray, fileName, download = True):
  ...
  st += "//Points\n"
  for i in range(num_vertices):
    rand1 = np.random.randint(60, 500)
    rand2 = np.random.randint(60, 500)
    st += "9 {}.0 {}.0 \n".format(rand1, rand2)
  st += "//Cluster is null\n//Historycounter\n-1\n//History"
...
# Reindex a 2d matrix, rows and columns simultaneously (or not, doesn't matter)
# If override is true, then we modify the original array, otherwise we return a new one
def Reindex(matrix: np.ndarray, new_index: list, override = False):
  # Do the sanity checks
  assert len(matrix.shape) == 2
  assert matrix.shape[0] == matrix.shape[1]
  assert len(new_index) == matrix.shape[0]
  assert sorted(new_index) == list(range(matrix.shape[0]))

  # Now reindex
  M = np.array(matrix, dtype = matrix.dtype) if override else matrix
  new_ind = list(range(matrix.shape[0]))
  M[[0, 1, 2, 3]] = M[new_index]
  M[:, [0, 1, 2, 3]] = M[:, new_index]
  return M
...
class TriangulationDiagram(TensorDiagram):
  ...
  def GenerateCluster(self, verbose = False, veryVerbose = False):
    ...
    # #############################################################################################################
    # Add all the exchange relations
    # #############################################################################################################
    addedRels = []
...
##########################################################################################
  ...
  def PrintQuiver(self, includeTriangulation = False, includeTrees = False, optimize = True):
    ...
    if includeTrees:
      for index, cv in self.cluster.clusterVariables:
        diagram.AddJInvariant(cv.j)
    elif includeTriangulation:
      diagram.Triangulate(self.triangulation)
      for edge in diagram.triangulation.edges:
        node1 = diagram.GetNodeFromID(edge[0], True)
        node2 = diagram.GetNodeFromID(edge[1], True)
        node1.connect(node2)
...
\t\\filldraw[color=black, fill=white!0, thin](0,0) circle (1);"""
    ...
    if optimize:
      ...
      for index, cv in self.cluster.clusterVariables + self.cluster.frozenVariables:
        # CV is a generator type object. Hence it has a intrinsic J structure
        new_coord = np.array([0, 0])
        if cv.t == 1:
          top_coord = np.array(self.GetNodeFromID(cv.top[0], True).coordinates)
          botttom_coord = np.array(self.GetNodeFromID(cv.bottom[0], True).coordinates)
          new_coord = 0.5 * (top_coord + botttom_coord)
        elif cv.t == 2:
          coord = np.array(self.GetNodeFromID(cv.bottom[0], True).coordinates)
          coord1 = np.array(self.GetNodeFromID(cv.bottom[1], True).coordinates)
          coord2 = np.array(self.GetNodeFromID(cv.bottom[2], True).coordinates)
          new_coord = (coord + coord1 + coord2) / 3
      # There are only type 1 and 2 generators in the initial seed so don't worry
        new_coord = np.round(new_coord, 5)
        cv.coordinates = tuple(new_coord)
...
signatures = [(), (8,), (7, 0), (6, 1), (5, 2), (4, 3), (6, 0, 0), (5, 1, 0), (4, 2, 0), (3, 3, 0), (2, 4, 0), (0, 1, 5), (4, 1, 1), (3, 2, 1), (2, 3, 1), (2, 2, 2),
(5, 0, 0, 0), (4, 1, 0, 0), (3, 2, 0, 0), (2, 3, 0, 0), (0, 0, 1, 4), (4, 0, 1, 0), (0, 3, 1, 1), (2, 2, 1, 0), (0, 1, 3, 1),
(3, 0, 2, 0), (2, 1, 2, 0), (0, 1, 2, 2), (0, 1, 1, 3), (2, 1, 1, 1)]