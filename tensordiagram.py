...
# Determine whether list1 and lsit 2 is the same up to reordering
def list_eq(list1, list2):
  return len(list1) == len(list2) and len(list1) == sum([1 for i, j in zip(list1, list2) if i == j])

def list_add(lis: list, elem):
  if Contains(lis, elem) == 0:
    lis.append(elem)
  return lis
...
def twoPointsIncircle(threshold = 0.05):
  x, y = -1, -1
...
class InvariantTerm:
  # Splices and processes a string equation into an invariant equation that python can read
  # Example [J((2,), (3,)), plus, J((4,), (5,)), equal, J((2, 3), (4, 5))]
...
class ClusterAlgebra:
  ...
  # Mutate at the k-th cluster variable, where the index is according to the enumeration stored in clusterVariables
  # TODO: Also change the cluster variable!!
  def Mutate(self, k: int):
    ...
    B = np.array(self.extended_exchange_matrix, dtype = int)

    # Screw it let's use a for loop
    for i in range(B.shape[0]):
      for j in range(B.shape[1]):
        # Equation 3.10 in Professor Ip's lecture notes
        self.extended_exchange_matrix[i, j] = -B[i, j] if k in set((i, j)) else B[i, j] + 0.5 * (np.abs(B[i, k]) * B[k , j] + B[i, k] * np.abs(B[k, j]))
...
class TriangulationDiagram(TensorDiagram):
  ...
  # Generate the quiver, add everything to the diagram and have some fun
  def Quiver(self):
    if not self.generated:
      self.GenerateCluster()
          
    # Now we use an extra diagram so if we include trees we will not make a hot mess
    diagram = TriangulationDiagram(self.signature)

    # Similar to what we do in the string cast method for tensor diagrams but we also loop through the cluster variables to print the quiver
    st = "\n\\centering\n\\begin{tikzpicture}[scale = 3]\n\t\\filldraw[color=black, fill=white!0, thin](0,0) circle (1);"
    # Same as the str method
    drawnLines = []
    st += diagram.__generateStrContents__()

    # We attempt to optimize the quiver by optimizing nodes on another diagram
    new_sig = [b for i in range(self.l)]
    diagram2 = TensorDiagram(new_sig)
    [diagram2.AddNode(b, False) for t in self.cluster.clusterVariables]

    # Since we are using 0-count for the exchange graph we want to subtract an additional 1 for the indexing
    num_var = 3 * self.l - 9

    # Now add all the edges in
    for i, cv in self.cluster.clusterVariables + self.cluster.frozenVariables:
      for j, cv2 in self.cluster.clusterVariables + self.cluster.frozenVariables:
        num_arrows = self.cluster.full_exchange_matrix[i, j]
        if num_arrows > 0:
          node_from = diagram2.GetNodeFromID(num_var - i, False)
          node_to = diagram2.GetNodeFromID(num_var - j, False)
          node_from.connect(node_to, ignore_full_connection = True)
    
    # Optimize the diagram and copy the coordinates back to the cluster variables
    # We need a little rotation since now the (frozen) variables are right on top of the boundary nodes
    # Specifically, if we have n frozen variables(boundary nodes), then we want to rotate by pi/n
    diagram2.OptimizeDiagram()
    for i, cv in self.cluster.clusterVariables + self.cluster.frozenVariables:
      corr_node = diagram2.GetNodeFromID(num_var - i, False)
      a = np.array(corr_node.coordinates)
      theta = PI/self.l
      x, y = np.cos(theta) * a[0] + np.sin(theta) * a[1], np.cos(theta) * a[1] - np.sin(theta)* a[0]
      cv.coordinates = (round(x, 5), round(y, 5))

    # Print a little padding under the frozen variables to make it look nicer
    for i, fv in self.cluster.frozenVariables:
      st += f"\n\t\\filldraw[color=white, fill=white] {fv.coordinates} circle (3pt);"

    # Now do the cluster variables
    for i, cv in self.cluster.clusterVariables:
      st += f"\n\t\\node[] at {cv.coordinates} {{\\({str(cv.j)}\\)}};"

    for i, fv in self.cluster.frozenVariables:
      st += f"\n\t\\node[] at {fv.coordinates} {{\\(\\boxed{{{str(fv.j)}}}\\)}};"
    
    # This part is the arrows
    for i, cv in self.cluster.clusterVariables + self.cluster.frozenVariables:
      for j, cv2 in self.cluster.clusterVariables + self.cluster.frozenVariables:
        num_arrows = self.cluster.full_exchange_matrix[i, j]
        if num_arrows > 0:
          A, B = stretch(cv.coordinates, cv2.coordinates, constant_pad = 0.125)
  ...
  def QMU(self, download = True):
    sig = ["b" if s == b else "w" for s in self.signature]
    si = "".join(sig)
    return GetQMU(self.cluster.exchange_matrix, si, download)
  ...
  def no_set(self, a):
    raise AssertionError("Quiver is read only!")
  
  def no_del(self):
    raise AssertionError("Quiver is read only!")
  
  quiver = property(Quiver, no_set, no_del)
